...
public class ContainerCyberChest extends Container
{
	...
	public ContainerCyberChest(InventoryPlayer playerInv, TileEntityChestInit tileEntityCloudChest, EntityPlayer player) 
	{
		this.chestInventory = tileEntityCloudChest;
		this.numRows =  tileEntityCloudChest.getSizeInventory() / 9;
		tileEntityCloudChest.openInventory(player);
		
		for(int i = 0; i < this.numRows; ++i)
		{
			for(int j = 0; j < 9; ++j)
			{
				this.addSlotToContainer(new Slot(tileEntityCloudChest, j + i*9, 8 + j*18, 18 + i*18));
			}
		}
