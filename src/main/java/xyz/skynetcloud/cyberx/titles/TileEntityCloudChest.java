...
public class TileEntityCloudChest extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return new ContainerCloudChest(playerInventory, this, playerIn);
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerCloudChest)
                {
                    if (((ContainerCloudChest)entityplayer.openContainer).getChestInventory() == this)
                    {
