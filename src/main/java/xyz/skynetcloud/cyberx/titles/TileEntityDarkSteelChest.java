...
public class TileEntityDarkSteelChest extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return new ContainerDarkSteelChest(playerInventory, this, playerIn);
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerDarkSteelChest)
                {
                    if (((ContainerDarkSteelChest)entityplayer.openContainer).getChestInventory() == this)
                    {
