...
public class TileEntityRubyChest extends TileEntityLockableLoot implements ITickable {
	private NonNullList<ItemStack> chestContents = NonNullList.<ItemStack>withSize(99, ItemStack.EMPTY);
	...
	{
		return this.hasCustomName() ? this.customName : "Cloud Chest";
	}
	...
	{
		return new ContainerRubyChest(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Main.MODID + ":dark_steel_chest_block"; 
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerRubyChest)
                {
                    if (((ContainerRubyChest)entityplayer.openContainer).getChestInventory() == this)
                    {
